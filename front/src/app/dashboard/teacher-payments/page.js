...
import {
  ...
  Loader2, RefreshCw, WalletCards, AlertCircle, Check
} from "lucide-react";
...
export default function TeacherPaymentsPage() {
  ...
  const loadDependencies = useCallback(async () => {
    try {
      const p1 = fetch(`${API}/teachers?pageSize=1000`, { headers: authHdr() }).then(r => r.json());
      const [r1] = await Promise.all([p1]);
      setTeachersList(Array.isArray(r1) ? r1 : r1.items ?? r1.data ?? []);
    } catch { }
  }, []);
  ...
  const loadPayments = useCallback(async (p = page) => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/teacher-payments?pageNumber=${p}&pageSize=${pageSize}`, { headers: authHdr() });
      ...
      toast("فشل تحميل مدفوعات الأساتذة", "error");
    } finally {
      setLoading(false);
    }
  }, [page, pageSize]);
  ...
  function openEditModal(item) {
    setSelected(item);
    setForm({
      teacherId: teachersList.find(t => `${t.firstName} ${t.lastName}` === item.teacherName)?.id || item.teacherId || "",
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const endpoint = modal === "add" ? `${API}/teacher-payments` : `${API}/teacher-payments/${selected.id}`;
      ...
      toast(modal === "add" ? "تم إضافة الدفعة للأستاذ بنجاح" : "تم تعديل الدفعة بنجاح");
      setModal(null);
      setSelected(null);
      loadPayments(page);
    } catch {
  ...
  async function handleDelete(id) {
    ...
    try {
      const res = await fetch(`${API}/teacher-payments/${id}`, { method: "DELETE", headers: authHdr() });
  ...
  return (
    <div className="stu-page">
      ...
      <div className="stu-header">
        <div>
          <h1 className="stu-title"><WalletCards size={22} /> مدفوعات الأساتذة</h1>
          <p className="stu-subtitle">إدارة مستحقات ومدفوعات الأساتذة وسجلات الرواتب</p>
        </div>
        <div className="stu-header-actions">
          <button className="stu-refresh-btn" onClick={() => loadPayments()} title="تحديث">
            <RefreshCw size={16} className={loading ? "spin" : ""} />
          </button>
          <button className="btn-gold" onClick={openAddModal}><Plus size={16} /> تسليم دفعة لأستاذ</button>
        </div>
      </div>
      ...
      <div className="stu-table-wrap">
        {loading ? (
          ...
          <div className="stu-empty"><WalletCards size={48} color="#333" /><p>لا توجد مدفوعات مسجلة</p></div>
        ) : (
          <table className="stu-table">
            ...
            <tbody>
              {payments.map((item) => (
                <tr key={item.id}>
                  <td style={{fontWeight: 'bold'}}>{item.teacherName}</td>
        ...
        <div className="stu-modal-overlay" onClick={() => setModal(null)}>
          <div className="stu-modal-card" onClick={e => e.stopPropagation()}>
            <div className="stu-modal-header">
              <span className="stu-modal-title">{modal === "add" ? "تسليم دفعة للأستاذ" : "تعديل دفعة الأستاذ"}</span>
              ...
              <form className="stu-form" onSubmit={handleSubmit}>
                <div className="stu-form-grid" style={{marginBottom: "1rem"}}>
                  ...
                  <div className="stu-fg">
                    <label>المبلغ الكلي المقرر للأستاذ *</label>
