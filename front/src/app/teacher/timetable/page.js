...
export default function TeacherTimetablePage() {
  ...
  return (
    <div>
      <h1 className="dash-page-title">جدول حصصي الأسبوعي</h1>
      <p style={{ color: "var(--gray)", marginBottom: "2rem", fontSize: "0.9rem" }}>
        <Calendar size={14} style={{ display: "inline", marginLeft: "4px" }} />
        الحصص التي تدرّسها هذا الأسبوع
      </p>

      {schedule.length === 0 ? (
        <div className="stu-empty">لم يتم إضافة جدول بعد</div>
      ) : (
                  ...
                  items.map(s => (
                    <div key={s.id} className="stu-slot-card">
                      <div className="stu-slot-subject">
                        <BookOpen size={14} style={{ marginLeft: 5, flexShrink: 0 }} />
                        {s.subjectName}
                      </div>
                      <div className="stu-slot-teacher">
                        <School size={12} style={{ marginLeft: 4, flexShrink: 0 }} />
                        {s.className}
                      </div>
                      <div className="stu-slot-time">
                        <Clock size={12} style={{ marginLeft: 4, flexShrink: 0 }} />
